...
class CPabe_BSW07(ABEnc):
    ...
    #@Output(pk_t, mk_t)
    def setup(self):                                                    #Nothing to change in this function
    ...
    #@Input(pk_t, mk_t, [str])
    #@Output(sk_t)                                                          #Nothing to change in this function
...
def main():
    ...
    attrs = ['ONE', 'TWO', 'THREE']                         #Change the attribute here      ['Ortho', 'Gynaecology', 'Billing']
    access_policy = '((four or three) and (three or one))'  #Change access policy            '(Ortho or Gynaecology or Billing)'
    ...
    (pk, mk) = cpabe.setup()                                #nth to change

    sk = cpabe.keygen(pk, mk, attrs)                        #nth to change
    #print("sk :=>", sk)


    rand_msg = groupObj.random(GT)                          #Massive change here: Point to a file or the contents of a file
